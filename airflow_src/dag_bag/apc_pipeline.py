...
from src.data_catalog.headings import APC_HEADINGS
...
apc_dag = DAG(
    ...
    default_args=args,
    description="""
        load a sample of apc zip file and transform to postgres. 
    """,
...
with apc_dag:
    ...
    setup_postgres_schema = PythonOperator(
        ...
        op_kwargs={
            'headings': APC_HEADINGS,
            'table_name': table_name,
            'DB_PROPERTIES': DB_PROPERTIES
        }
    )

    sub_dags = create_all_subdag_operators(apc_dag, apc_zip_files, DB_PROPERTIES, table_name, runtime_limit, batch_size)
